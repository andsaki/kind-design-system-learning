...
- [WCAG 2.1 達成基準 2.1.1 Keyboard（キーボード操作可能）](https://www.w3.org/TR/WCAG21/#keyboard) で必須となっている通り、スクリーンリーダーやスイッチ入力など支援技術はキーボードイベントを前提にしています。マウスやトラックパッドだけに依存すると、これらのユーザーは操作できません。
- トラックパッドはポインタ操作の一種にすぎず、ポインタ制御が難しいユーザーや手を大きく動かせないユーザーはキーボードに頼ります。よって「ポインタでも操作できる」「キーボードでも操作できる」の両方を満たす設計が求められます。
- キーボード操作は、スクリーンリーダーやスイッチ入力など支援技術の基本であり、マウスやトラックパッドだけに依存すると多くのユーザーが操作できません。国際基準（WCAG）でも「キーボードのみで完結できること」が必須要件として定義されています。
- トラックパッドはポインタ操作の一種にすぎず、ポインタ制御が難しいユーザーや手を大きく動かせないユーザーはキーボードに頼ります。よって「ポインタでも操作できる」「キーボードでも操作できる」の両方を満たす設計が求められます。
